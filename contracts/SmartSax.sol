...
contract SmartSax is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Jazzs to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Jazzs distribution occurs.
        uint256 accJazzPerShare; // Accumulated Jazzs per share, times 1e12. See below.
    }
    ...
    // The block number when Jazz mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 jazzReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake Jazz tokens to SmartYeti
